...
- CacheIndex filters expired observations, deduplicates non-empty event IDs, and requires positive producer sequences to advance per Worker. Events rejected by package validation do not consume an event ID or sequence; valid stale events retain their immutable event ID. Sequence zero is an internal unordered local mode and is rejected by the admin event API.
...
- A Worker ID cannot change URL or topology in place. Its old registration must be safely drained and removed; removal purges that ID's cache observations before a later explicit registration. Worker endpoints must use HTTP(S), and the Router rejects rather than follows upstream redirects.
- File-backed and S3-compatible stores publish payload before metadata. Across every tier in one metadata database, an active BlockKey accepts an idempotent replay only when its bytes and payload descriptor agree; a conflicting payload or descriptor is rejected. Orphan bytes from a failed metadata commit stay invisible until an explicit recovery policy is implemented.
