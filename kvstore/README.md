...
- An active `BlockKey` is immutable: storing the same bytes and payload descriptor is idempotent, while conflicting bytes or dtype/shape/token metadata require explicit tombstone-backed deletion before a new publication.
...
- Load uses `lookup_and_acquire`, revalidates persisted file/segment locations against the configured root and S3 locations against the configured bucket/prefix, then checks the bounded record structure, identity, payload length, and SHA-256 before release.
...
- S3-compatible HEAD/GET timeout or unavailability and record/content corruption are separate failure classes. An unavailable S3 observation does not hide a healthy local location; with no healthy location the tier manager returns a miss-like recompute result. Corruption is surfaced and is not automatically converted to recompute.
