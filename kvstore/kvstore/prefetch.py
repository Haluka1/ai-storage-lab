...
class Prefetcher:
    def __init__(self, store, max_workers: int = 2, max_queue: int = 64):
        if max_workers <= 0:
        ...
        self._seen: set[tuple[BlockKey, TierName]] = set()
        self._submitted = 0
        self._dropped = 0
        self._failed = 0

    def submit(self, key: BlockKey, target_tier: TierName) -> PrefetchSubmitResult:
        work = (key, target_tier)
        with self._lock:
    ...
    def _run_one(self, work: tuple[BlockKey, TierName]) -> None:
        key, target_tier = work
        self.store.prefetch([key], target_tier=target_tier)

    def _done(self, work: tuple[BlockKey, TierName], future: Future) -> None:
